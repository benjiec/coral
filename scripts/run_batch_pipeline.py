...
def check_existing_interpro_results(accession: str, interpro_dir: str) -> bool:
    """Check if InterProScan TSV results already exist for the given accession."""
    tsv_path = os.path.join(interpro_dir, f"{accession}.tsv")
    return os.path.exists(tsv_path)
        ...
        try:
            ...
            if should_skip_interpro:
                ...
                if not os.path.exists(tsv_path):
                    warning = f"Warning: InterProScan TSV output not found for {accession} despite skip flag"
                    print(warning, file=sys.stderr)
                    warnings.append((accession, warning))
                    continue
                skipped += 1
            else:
                tsv_path = run_interproscan(fasta_path, accession, INTERPRO_OUTPUTS)
